...
ui_blast2 <- function() {
  fluidPage(
    ...
    sidebarPanel(
      ...
      selectInput("Searchdatabase2", "Select database type:", c(custom_db2,"All-database", "Reference", 'Selected'), selected = "blast_db.fa"),
      selectInput("Searchbtype2", "Blast type:", c('blastn', "tblastn"), selected = "blastn"),
      number_EV <- numericInput(
        "Evaluesinsert2",
        "Evalues",
        min = 1e-10, max = 0.1, step = 0.001, value = 0.01 # default is answer = 0.01
      ),
      number_Maxtarget <- numericInput(
        "Maxtargetseq", 
        "Max Target sequences",min = 10, max = 1000, step = 10, value = 10 # default is answer = 10
      ),
      number_Maxhsps <- numericInput(
        "Maxhsps", 
        "Max hsps",min = 1, max = 5, step = 1, value = 1 # default is answer = 1
      ),
